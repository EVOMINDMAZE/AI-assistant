...
/** Shared DeepSeek client — used by the `deepseekModel` adapter below and
 *  by the `runAgentDirect` bypass in `direct-runner.ts`. */
...
async function callWithRetry(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      return await deepseekClient.chat.completions.create(body);
    } catch (err: any) {
      lastErr = err;
...
export function itemsToMessages(items: AgentInputItem[]): OpenAI.Chat.Completions.ChatCompletionMessageParam[] {
